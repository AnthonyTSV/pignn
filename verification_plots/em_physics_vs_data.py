...
SAVE_DIR = PROJECT_ROOT / "verification_plots/em_physics_vs_data"
SAVE_DIR.mkdir(exist_ok=True, parents=True)

PI_16_SOURCE_DATA = Path(
    PROJECT_ROOT
    / "verification_plots/eddy_current_generalisation_mu_r_sigma/relative_errors.npz"
)
...
def _checkpoint_path(experiment: Experiment) -> Path:
    if experiment.num_train == 16 and experiment.uses_physics:
        return (
            PROJECT_ROOT
    ...
    model_dir = (
        f"results/physics_informed/em_ablation_{experiment.num_train}_pnts_"
...
def _evaluate(
    ...
    config = {
        ...
        "save_dir": str(SAVE_DIR / "evaluation_runs" / experiment.file_stem),
...
def generate_experiment_data(experiment: Experiment, force: bool = False) -> Path:
    ...
    if (
        experiment.num_train == 16
        and experiment.uses_physics
        and PI_16_SOURCE_DATA.is_file()
        and not force
    ):
        print(f"{experiment.file_stem}: reusing {PI_16_SOURCE_DATA}")
        return PI_16_SOURCE_DATA
...
def plot_comparison_heatmaps(
    ...
    fig.savefig(SAVE_DIR / "em_heatmaps_physics_vs_data.pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def plot_saved_data(experiments: list[Experiment]) -> None:
    ...
    for experiment in experiments:
        plot_experiment_heatmap(experiment, vmax=shared_vmax[experiment.num_train])
    plot_comparison_heatmaps(experiments, vmax_by_num_train=shared_vmax)
    write_summary_tables(experiments)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate and plot EM physics-loss ablation heatmaps."
    )
    parser.add_argument(
        "--mode",
