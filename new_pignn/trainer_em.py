...
class PIMGNTrainerEM:
    ...
    def train_step(self, problem_idx, prediction=None, ground_truth=None, data_weight=0.0):
        """
        ...
        if ground_truth is not None and data_weight > 0:
            ...
            data_loss_A = ((pred_A_real - gt_A_real)**2 + (pred_A_imag - gt_A_imag)**2).mean() / (gt_A_norm**2 / n_total)
            ...
            gt_phi_norm = torch.sqrt((gt_phi_real**2 + gt_phi_imag**2).sum()).clamp_min(1e-10)
            n_phi = len(coil_idx)
            data_loss_phi = ((pred_phi_real_coil - gt_phi_real)**2 + (pred_phi_imag_coil - gt_phi_imag)**2).mean() / (gt_phi_norm**2 / n_phi)
        ...
        for prob_idx in train_problems_indices:
            if self.problems[prob_idx].complex:
                ...
                self._ground_truth_tensors[prob_idx] = {
                    'A_real': torch.tensor(A_dofs.real, dtype=torch.float64, device=self.device),
                    'A_imag': torch.tensor(A_dofs.imag, dtype=torch.float64, device=self.device),
                    'phi_real': torch.tensor(phi_dofs.real, dtype=torch.float64, device=self.device),
                    'phi_imag': torch.tensor(phi_dofs.imag, dtype=torch.float64, device=self.device),
                    'coil_node_indices': torch.tensor(fem_s.coil_node_indices, dtype=torch.long, device=self.device),
                }
            else:
