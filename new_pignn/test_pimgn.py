    ...
    def compute_physics_informed_loss(
        ...
        for t_idx in range(self.time_window):
            ...
            if dirichlet_vals_tensor is not None:
                prediction_full_t[dirichlet_mask] = dirichlet_vals_tensor[dirichlet_mask]
            ...
            for problem_idx in val_problems_indices:
                ...
                for step_idx in range(0, validation_steps, self.time_window):
                    ...
                    free_data, node_mapping, free_aux = (
                        graph_creator.create_free_node_subgraph(
                            full_graph=data, aux=aux
                        )
                    )
                    free_data = free_data.to(self.device)
...
def train_pimgn_on_single_problem(resume_from: str = None):
    ...
    config = {
        ...
        "save_dir": "results/physics_informed/verification_test_problem_3_maxh_0.1",
        "resume_from": resume_from,  # Path to checkpoint to resume from
    }
    _run_single_problem_experiment(problem, time_config, config, "Test problem 3")
...
def train_pimgn_on_industrial_problem():
    problem, time_config = create_industrial_heating_problem(
        maxh=3e-3
    )
    config = {
        "epochs": 100,
