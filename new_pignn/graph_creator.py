...
def _is_quadratic_fespace(fes: Optional[ng.FESpace]) -> bool:
    """Return True if the provided FESpace is at least second order."""
    if fes is None:
        return False
    # NGSolve H1 high-order spaces expose globalorder
    order = getattr(fes, "globalorder", None)
...
def _classify_node_types(
    ...
    pnum_to_idx: Dict[int, int],
    *,
    edge_to_mid_idx: Optional[Dict[Tuple[int, int], int]] = None,
    n_total_nodes: Optional[int] = None,
) -> torch.Tensor:
    """
    ...
    """
    n_vertex_nodes = len(pnum_to_idx)
    n_nodes = int(n_total_nodes) if n_total_nodes is not None else n_vertex_nodes
    ...
    for seg in segments_1d:
        ...
        if bc_code in neumann_bcnr:
            ...
            if edge_to_mid_idx is not None and len(vertex_indices) == 2:
                a, b = vertex_indices
                mid = edge_to_mid_idx.get((a, b)) or edge_to_mid_idx.get((b, a))
                if mid is not None:
                    node_types[mid] = 1
...
def _extract_undirected_vertex_edges(
    mesh: Mesh, pnum_to_idx: Dict[int, int]
) -> set[Tuple[int, int]]:
    """Extract unique undirected edges between vertex nodes.

    Prefer `mesh.edges` (NGSolve topology) which is robust in 2D/3D and aligns with H1 edge dofs.
    Fallback to element-based extraction if `mesh.edges` is unavailable.
    """
    ...
    if mesh_edges is not None:
        try:
            ...
            # Fall back to element-based extraction
            edges.clear()

    n_vertex_nodes = len(pnum_to_idx)

    # 2D surface elements / 2D volume elements (triangles). For triangles, all pairs are true edges.
    for elem in mesh.ngmesh.Elements2D():
        vertex_indices = _normalize_ids_to_idx(
            elem.vertices, pnum_to_idx, n_vertex_nodes, context="2D_elements"
        )
        if len(vertex_indices) == 3:
            ...
            for i in range(len(vertex_indices)):
                u = vertex_indices[i]
                v = vertex_indices[(i + 1) % len(vertex_indices)]
                if u != v:
                    edges.add((min(u, v), max(u, v)))

    # 1D boundary segments in 2D
    for elem in mesh.ngmesh.Elements1D():
        vertex_indices = _normalize_ids_to_idx(
            elem.vertices, pnum_to_idx, n_vertex_nodes, context="1D_elements"
        )
...
def _map_h1_dof_vector_to_graph_nodes(
    ...
    *,
    ...
    """Map an H1 GridFunction dof-vector (length `fes.ndof`) to graph-node values.

    The graph node ordering is assumed to be:
      - vertex nodes at indices matching `vertex.nr` (0-based)
      - optional mid-edge nodes at indices given by `edge_to_mid_idx[(min(v0,v1), max(v0,v1))]`

    This supports order-1 (vertex dofs) and order-2 (vertex + single edge dof).
    """
    arr = np.asarray(dof_values).reshape(-1)
    if arr.shape[0] != int(fes.ndof):
        raise ValueError(
            f"Expected dof vector of length fes.ndof={int(fes.ndof)}, got {arr.shape[0]}"
        )

    out = np.zeros((n_total_nodes,), dtype=arr.dtype)

    # Vertex dofs
    for v in mesh.vertices:
        dofs = fes.GetDofNrs(ng.NodeId(ng.VERTEX, int(v.nr)))
        if dofs:
            out[int(v.nr)] = arr[int(dofs[0])]

    # Edge dofs (order >= 2)
    if edge_to_mid_idx is not None:
        for e in mesh.edges:
            dofs = fes.GetDofNrs(ng.NodeId(ng.EDGE, int(e.nr)))
            if not dofs:
                continue
            if len(dofs) != 1:
                raise ValueError(
                    "Only order-2 (one dof per edge) is supported for dof-vector mapping. "
                    f"Edge {int(e.nr)} returned dofs={tuple(dofs)}"
                )
            v0, v1 = e.vertices
            key = (min(int(v0.nr), int(v1.nr)), max(int(v0.nr), int(v1.nr)))
            mid = edge_to_mid_idx.get(key)
            if mid is None:
                raise KeyError(
                    f"Edge midpoint index not found for edge vertices {key}."
                )
            out[mid] = arr[int(dofs[0])]

    return out


def _augment_positions_with_edge_midpoints(
    pos_vertices: np.ndarray, undirected_edges: set[Tuple[int, int]]
) -> tuple[np.ndarray, Dict[Tuple[int, int], int]]:
    """Add one midpoint node per undirected edge (i,j) and return extended pos and mapping."""
    pos_list = [pos_vertices]
    edge_to_mid_idx: Dict[Tuple[int, int], int] = {}
    n_vertices = pos_vertices.shape[0]
    dim = pos_vertices.shape[1]
    mids = np.zeros((len(undirected_edges), dim), dtype=pos_vertices.dtype)

    for k, (i, j) in enumerate(sorted(undirected_edges)):
        mids[k] = 0.5 * (pos_vertices[i] + pos_vertices[j])
        edge_to_mid_idx[(i, j)] = n_vertices + k

    pos_list.append(mids)
    return np.vstack(pos_list), edge_to_mid_idx


def _build_quadratic_fem_connectivity(
    mesh: Mesh,
    pnum_to_idx: Dict[int, int],
    ...
    """Build quadratic (order-2) FEM connectivity.

    Important: For H1 order 2, the local stencil couples vertex DOFs and mid-edge DOFs
    within each element. A connectivity that only splits edges (vertex<->mid<->vertex)
    is too sparse and tends to underperform specifically on mid-edge nodes.

    This builder therefore creates a fully-connected directed subgraph per element using
    the 6-node (tri) / (vertices + mid-edge) set, which better matches the sparsity
    pattern of FEM matrices.
    """
    edges_set: set[Tuple[int, int]] = set()
    n_vertex_nodes = len(pnum_to_idx)

    def add_fully_connected(nodes: List[int]) -> None:
    ...
    # 2D elements: connect all vertex + mid-edge nodes within each surface element
    for elem in mesh.ngmesh.Elements2D():
        vertex_indices = _normalize_ids_to_idx(
            elem.vertices, pnum_to_idx, n_vertex_nodes, context="2D_elements"
        )
        ...
        mids: List[int] = []
        # For polygonal elements, treat consecutive vertex pairs as true edges
        for k in range(len(vertex_indices)):
            u = vertex_indices[k]
            v = vertex_indices[(k + 1) % len(vertex_indices)]
            if u == v:
                continue
            key = (min(u, v), max(u, v))
            mid = edge_to_mid_idx.get(key)
        ...
        add_fully_connected(vertex_indices + mids)

    # 1D boundary elements: connect endpoints and (if present) the mid-edge node
    for seg in mesh.ngmesh.Elements1D():
        vertex_indices = _normalize_ids_to_idx(
            seg.vertices, pnum_to_idx, n_vertex_nodes, context="1D_elements"
        )
        if len(vertex_indices) != 2:
            continue
        u, v = vertex_indices
        ...
        if mid is not None:
            nodes.append(mid)
        add_fully_connected(nodes)

    # Safety: ensure at least the split-edge connectivity exists even if element iteration failed
    # (e.g., unusual meshes). This keeps the graph connected for mid-edge nodes.
    if not edges_set:
        for i, j in undirected_edges:
            mid = edge_to_mid_idx[(i, j)]
            edges_set.add((i, mid))
            edges_set.add((mid, i))
            edges_set.add((mid, j))
            edges_set.add((j, mid))
...
def _extend_scalar_nodal_field_to_quadratic(
    ...
    """If field is vertex-only, extend to include mid-edge nodes via endpoint averaging."""
    if field is None:
        return None
    arr = np.asarray(field)
    if arr.ndim != 1:
        raise ValueError(f"Expected a 1D nodal array, got shape={arr.shape}.")
    if arr.shape[0] == n_total_nodes:
        return arr
    if arr.shape[0] != n_vertex_nodes:
        raise ValueError(
            f"Nodal array length mismatch: got {arr.shape[0]}, expected {n_vertex_nodes} (vertex-only) or {n_total_nodes} (quadratic)."
        )

    out = np.zeros((n_total_nodes,), dtype=arr.dtype)
    out[:n_vertex_nodes] = arr
    for i, j in undirected_edges:
        mid = edge_to_mid_idx[(i, j)]
        out[mid] = 0.5 * (arr[i] + arr[j])
    return out
...
def _build_node_features(
    ...
    Build node feature matrix according to PI-MGN methodology.
        Features: [one_hot_node_type(4), T_current(?), t_scalar(1), material(1), neumann_value(?), dirichlet_value(?), robin_h(1), robin_amb(1), source(?)]

        Notes on complex values:
            - This codebase uses real-valued PyTorch layers (nn.Linear, etc.).
            - Therefore, any complex-valued fields are represented as two real channels
                (real, imag) and NOT as torch.complex tensors.
    """
    if device is None:
    ...
    # Current solution field (can be complex; represent as [real, imag])
    if T_current is not None:
        if np.iscomplexobj(T_current):
            T_real = torch.tensor(T_current.real, dtype=torch.float32, device=device)
            T_imag = torch.tensor(T_current.imag, dtype=torch.float32, device=device)
            T_tensor = torch.stack([T_real, T_imag], dim=1)
        else:
    ...
    # Neumann boundary values (can be complex; represent as [real, imag])
    if neumann_values is not None:
        if np.iscomplexobj(neumann_values):
            n_real = torch.tensor(
                np.asarray(neumann_values).real, dtype=torch.float32, device=device
            )
            n_imag = torch.tensor(
                np.asarray(neumann_values).imag, dtype=torch.float32, device=device
            )
            neumann_tensor = torch.stack([n_real, n_imag], dim=1)
        else:
    ...
    # Dirichlet boundary values (can be complex; represent as [real, imag])
    if dirichlet_values is not None:
        if np.iscomplexobj(dirichlet_values):
            d_real = torch.tensor(
                np.asarray(dirichlet_values).real, dtype=torch.float32, device=device
            )
            d_imag = torch.tensor(
                np.asarray(dirichlet_values).imag, dtype=torch.float32, device=device
            )
            dirichlet_tensor = torch.stack([d_real, d_imag], dim=1)
        else:
    ...
    # Source term values (can be complex; represent as [real, imag])
    if source_values is not None:
        if isinstance(source_values, torch.Tensor):
            src = source_values.detach().clone().to(device=device)
            if torch.is_complex(src):
                source_tensor = torch.stack([src.real, src.imag], dim=1).to(
                    dtype=torch.float32
                )
            else:
                source_tensor = src.to(dtype=torch.float32).unsqueeze(1)
        else:
            if np.iscomplexobj(source_values):
                s_real = torch.tensor(
                    np.asarray(source_values).real, dtype=torch.float32, device=device
                )
                s_imag = torch.tensor(
                    np.asarray(source_values).imag, dtype=torch.float32, device=device
                )
                source_tensor = torch.stack([s_real, s_imag], dim=1)
            else:
...
def _build_edge_features(
    ...
    if edge_index.numel() == 0:
        # [relative_pos(dim), euclidean_dist(1), diff(?)] where diff is 1 (real) or 2 (real, imag)
        diff_dim = 2 if (T_current is not None and np.iscomplexobj(T_current)) else 1
        return torch.empty(
            (0, pos.shape[1] + 1 + diff_dim), dtype=torch.float32, device=device
        )
    ...
    # Solution difference (if available). For complex fields, use two channels [real, imag].
    if T_current is not None:
        if isinstance(T_current, np.ndarray) and np.iscomplexobj(T_current):
            T_real = torch.tensor(T_current.real, dtype=torch.float32, device=device)
            T_imag = torch.tensor(T_current.imag, dtype=torch.float32, device=device)
            src_real = T_real[edge_index[0]]
            dst_real = T_real[edge_index[1]]
            src_imag = T_imag[edge_index[0]]
            dst_imag = T_imag[edge_index[1]]
            diff_real = (dst_real - src_real).unsqueeze(1)
            diff_imag = (dst_imag - src_imag).unsqueeze(1)
            temp_diff = torch.cat([diff_real, diff_imag], dim=1)
        else:
...
class GraphCreator:
    ...
    def create_graph(
        ...
        # 1. Build node positions and index mapping (vertex nodes)
        pos_vertices, pnum_to_idx = _build_point_index_map(self.mesh)
        n_vertex_nodes = len(pos_vertices)

        # Optionally augment to quadratic nodes using the FEM edges (one mid-edge node per edge)
        is_quadratic = (
            self.connectivity_method.lower() == "fem"
            and _is_quadratic_fespace(self.fes)
        )
        undirected_edges: Optional[set[Tuple[int, int]]] = None
        edge_to_mid_idx: Optional[Dict[Tuple[int, int], int]] = None

        if is_quadratic:
            undirected_edges = _extract_undirected_vertex_edges(self.mesh, pnum_to_idx)
            pos, edge_to_mid_idx = _augment_positions_with_edge_midpoints(
                pos_vertices, undirected_edges
            )
        else:
            pos = pos_vertices
        ...
        node_types = _classify_node_types(
            ...
            self.robin_names or [],
            pnum_to_idx,
            edge_to_mid_idx=edge_to_mid_idx,
        ...
        if self.connectivity_method.lower() == "fem":
            if is_quadratic:
                assert undirected_edges is not None and edge_to_mid_idx is not None
                edge_index = _build_quadratic_fem_connectivity(
                    self.mesh,
                    pnum_to_idx,
                    undirected_edges,
                    edge_to_mid_idx,
        ...
        # Map/extend nodal inputs to match graph nodes
        fes_ndof = int(getattr(self.fes, "ndof", -1)) if self.fes is not None else -1

        def normalize_field(
            field: Optional[np.ndarray], *, name: str
        ) -> Optional[np.ndarray]:
            if field is None:
                return None
            arr = np.asarray(field).reshape(-1)

            # If caller passed a GridFunction dof-vector, map it to graph nodes
            if (
                self.connectivity_method.lower() == "fem"
                and self.fes is not None
                and fes_ndof > 0
                and arr.shape[0] == fes_ndof
            ):
                if is_quadratic and edge_to_mid_idx is not None:
                    if n_nodes != fes_ndof:
                        raise ValueError(
                            f"[{name}] fes.ndof={fes_ndof} does not match graph n_nodes={n_nodes}. "
                            "This usually means the graph does not include all H1 dofs (e.g., order>2)."
                        )
                    return _map_h1_dof_vector_to_graph_nodes(
                        arr,
                        mesh=self.mesh,
                        fes=self.fes,
                        n_total_nodes=n_nodes,
                ...
                # Order-1 case (vertex dofs only)
                if not is_quadratic:
                    if n_nodes != n_vertex_nodes:
                        raise ValueError(
                            f"[{name}] Internal error: expected n_nodes==n_vertex_nodes for linear graph."
                        )
                    return _map_h1_dof_vector_to_graph_nodes(
                        arr,
                        mesh=self.mesh,
                        fes=self.fes,
                        n_total_nodes=n_nodes,
                        edge_to_mid_idx=None,
                    )

                raise ValueError(
                    f"[{name}] Got a dof-vector (len={arr.shape[0]}) but cannot map it for connectivity_method='{self.connectivity_method}'"
                )

            # Otherwise treat as nodal array on vertices or on full graph
            if is_quadratic:
                assert undirected_edges is not None and edge_to_mid_idx is not None
                return _extend_scalar_nodal_field_to_quadratic(
                    arr, n_vertex_nodes, n_nodes, undirected_edges, edge_to_mid_idx
                )

            # Linear graph: accept either full-length or vertex-length (same here)
            if arr.shape[0] != n_nodes:
                raise ValueError(
                    f"[{name}] Expected nodal array of length {n_nodes} for this graph, got {arr.shape[0]}."
                )
            return arr

        T_current = normalize_field(T_current, name="T_current")
        material_node_field = normalize_field(
            material_node_field, name="material_node_field"
        )
        neumann_values = normalize_field(neumann_values, name="neumann_values")
        dirichlet_values = normalize_field(dirichlet_values, name="dirichlet_values")
        source_values = normalize_field(source_values, name="source_values")

        if robin_values is not None:
            h_vals, amb_vals = robin_values
            h_vals = normalize_field(h_vals, name="robin_h")
            amb_vals = normalize_field(amb_vals, name="robin_amb")
    ...
    def create_neumann_values(
        ...
        edge_to_mid_idx = aux_data.get("edge_to_mid_idx")
        ...
        for boundary_name in neumann_names:
            ...
            boundary_nodes = _get_nodes_on_boundary(
                mesh, boundary_name, pnum_to_idx, edge_to_mid_idx=edge_to_mid_idx
            )
    ...
    def visualize_graph(
        ...
        # fig, ax1 = plt.subplots(1, 1, figsize=(figsize[0] / 4, figsize[1]))
